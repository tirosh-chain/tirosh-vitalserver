...
def write_runtime_observation(
    ...
    *,
    ...
    state = runtime_observation_document(
...
def runtime_observation_document(
    *,
    guest_http: RuntimeHTTPProbeStatus | str | None = None,
    redis_ui_http: RuntimeHTTPProbeStatus | str | None = None,
    swagger_ui_http: RuntimeHTTPProbeStatus | str | None = None,
) -> GuestRuntimeObservation:
    return collector.collect_runtime_observation(
        guest_http=guest_http,
        redis_ui_http=redis_ui_http,
        swagger_ui_http=swagger_ui_http,
    )
...
def write_runtime_observation_document(
    runtime_observation: Path,
    ...
    *,
    ...
    runtime_observation.parent.mkdir(parents=True, exist_ok=True)
    temporary = runtime_observation.with_suffix(runtime_observation.suffix + ".tmp")
    temporary.write_text(
        json.dumps(state.as_json(), indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    os.replace(temporary, runtime_observation)

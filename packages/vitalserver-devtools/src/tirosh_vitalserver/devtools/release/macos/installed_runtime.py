...
def run_installed_status(args: Namespace) -> int:
    status = installed_status(args)
    return 1 if args.fail_on_unhealthy and status != 0 else 0


def run_installed_health(args: Namespace) -> int:
    status = installed_status(args)
    health = installed_health(args.config, args.proxy_port)
    return status or health


def installed_status(args: Namespace) -> int:
    root = repo_root()

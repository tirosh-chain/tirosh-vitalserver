...
def test_linux_runtime_bundle_is_complete_and_deterministic(tmp_path: Path) -> None:
    ...
    controller.mkdir()
    (controller / "__init__.py").write_text("", encoding="utf-8")
    (controller / "api.py").write_text("VALUE = 1\n", encoding="utf-8")
    ...
    with tarfile.open(first, "r:gz") as archive:
        ...
        assert {
            ...
            "VitalServer-Linux/bin/vitalserver-runtime-controller.pyz",
        ...
        controller_archive = archive.extractfile(
            "VitalServer-Linux/bin/vitalserver-runtime-controller.pyz"
        )
        assert controller_archive is not None
        with zipfile.ZipFile(io.BytesIO(controller_archive.read())) as zip_app:
            assert "__main__.py" in zip_app.namelist()
            assert "tirosh_guest_tools/api.py" in zip_app.namelist()
...
def test_linux_runtime_controller_zipapp_runs_without_online_install(
    tmp_path: Path,
) -> None:
    spec = importlib.util.spec_from_file_location("linux_bundle_builder", BUILDER)
    assert spec is not None and spec.loader is not None
    ...
    source = (
        ROOT
        / "packages/vitalserver-guest-tools/src/tirosh_guest_tools"
    )
    first = tmp_path / "controller-first.pyz"
    second = tmp_path / "controller-second.pyz"

    builder._write_runtime_controller_zipapp(source, first)
    builder._write_runtime_controller_zipapp(source, second)
    ...
    assert "Run the Guest Control API" in result.stdout
...
def test_linux_installer_gives_expensive_runtime_reads_an_explicit_timeout() -> None:
    ...
    assert 'parser.add_argument("--http-timeout-seconds", type=int, default=60)' in acceptance
...
def test_linux_installer_migrates_and_rolls_back_platform_delivery_config() -> None:
    ...
    assert '"rollbackTool": "/opt/vitalserver/current/tools/rollback-linux.py"' in installer
    assert '"uninstallTool": "/opt/vitalserver/current/tools/uninstall-linux.py"' in installer
    assert '"supportExportTool": "/opt/vitalserver/current/tools/support-export-linux.py"' in installer
    assert (
        'install -m 0755 packaging/support-export-linux.py '
        '"$staging_root/tools/support-export-linux.py"'
    ) in installer
    assert 'install -m 0755 packaging/support-export-linux.py "$staging_root/tools/support-export-linux.py"' in installer
...
def test_linux_update_uses_explicit_non_nested_support_acceptance_mode() -> None:
    ...
    assert '--acceptance-support-export-mode "$acceptance_support_export_mode"' not in installer
...
def test_linux_support_export_preserves_each_source_result(monkeypatch, tmp_path: Path) -> None:
    spec = importlib.util.spec_from_file_location("support_export_linux", SUPPORT_EXPORTER)
    assert spec is not None and spec.loader is not None
    ...
    assert workflow["artifact"]["sha256"] == _sha256(
        Path(workflow["artifact"]["path"])
    )
    with tarfile.open(workflow["artifact"]["path"], "r:gz") as archive:
        ...
        property_argument = next(
            argument for argument in status["command"] if argument.startswith("--property=")
        )
...
def test_linux_uninstall_reinstall_acceptance_proves_all_persistent_owners() -> None:
    acceptance = (
        ROOT
        / "apps/vitalserver-platform-agent/packaging/linux/acceptance-uninstall-reinstall-linux.py"
    ).read_text(encoding="utf-8")

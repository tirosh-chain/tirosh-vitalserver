...
def test_reads_runtime_state_contract_fields(tmp_path):
    vm_home = tmp_path / "vm"
...
def test_reports_missing_required_runtime_state_field(tmp_path):
    state_file = runtime_state_file(tmp_path / "vm")
    state_file.parent.mkdir(parents=True)

...
- bed/recorder domain state 생성
- upstream Socket.IO event 의미 변경
- upstream VitalServer 코드 patch
- audit 실패를 VitalServer 기능 실패로 승격
...
### 6-2. Redis IP sync status

`recorders[].redisIpSync.status` 값은 다음 의미를 가집니다.

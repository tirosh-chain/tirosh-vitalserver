...
`make vm-pkg`는 package에 넣을 macOS nginx bundle까지 포함해서 만듭니다. `make vm-nginx-bundle`은 기본적으로 build machine의 nginx를 release artifact cache로 복사한 뒤, `apps/vitalserver-vm-launcher/Support/Build/vm-build.toml`의 `[nginx]` 설정으로 bundle을 만듭니다. 기본 artifact 경로는 아래입니다.

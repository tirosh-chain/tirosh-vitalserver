...
`runtime-status.json` can show `vm-runtime-state-missing`, stale VM lifecycle, failed host proxy HTTP, and missing container observation while the VM launchd service is still running.

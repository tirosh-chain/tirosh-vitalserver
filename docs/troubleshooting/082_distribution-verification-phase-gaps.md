...
Distribution targets historically grew around artifact type instead of release phase. That made build, artifact verification, install verification, update smoke, and installed runtime smoke easy to confuse. The missing phase is not a runtime state and must not be hidden by a successful build target.

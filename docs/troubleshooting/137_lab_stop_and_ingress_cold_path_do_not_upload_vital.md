# Lab Stop And Recorder Ingress Cold Path Do Not Upload Vital Files
...
- A Product Lab session or recorder changes to `stopped`, but no generated `.vital` file appears in VitalServer My Files.
...
Product Lab stop closed Socket.IO connections but did not send an explicit archive finalization command. Recorder
...
- Product Lab stop sends an explicit vrcode-scoped finalization request to recorder ingress.
...
- Lifecycle owners must emit explicit finalization commands; disconnect must not be promoted to recorder stopped.
...
- Byte cursor tests must cover the first append, incremental export, another active recorder, and explicit Lab stop.
...
- 2026-07-15: Added explicit Product Lab finalization, recorder-scoped cold-path jobs, byte-window recovery, and
  current-job-only uploads.

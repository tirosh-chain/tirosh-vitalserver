...
3. 수정된 패키지로 다시 설치합니다. 수정된 패키지는 progress/command event 기록 시 전체 health probe를 반복하지 않고, `postinstall` 자체도 300초 guard를 둡니다.
4. 이전 실패로 부분 설치물이 남아 있으면 uninstall/clean 정책에 맞춰 정리한 뒤 재설치합니다.

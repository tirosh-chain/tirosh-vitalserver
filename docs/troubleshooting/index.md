...
| TS-211 | Guest update owner operation이 `pending`/`running`에서 끝나지 않음 | Update / Guest execution | resolved | [Guest update owner operation never executes](211_guest_update_owner_operation_never_executes.md) |
| TS-212 | Guest-owned layer executor가 owner terminal state 없이 성공을 보고함 | Update / Host-to-Guest execution | resolved | [Guest-owned layer executor reports false success](212_guest_owned_layer_executor_reports_false_success.md) |
| TS-213 | Fresh install이 Container/Guest Runtime 초기 owner를 provision하지 않음 | Packaging / Guest bootstrap / Update | resolved | [Fresh install does not provision update owners](213_fresh_install_update_owners_not_provisioned.md) |
...
| TS-191 | PKG/DMG build에 stable updater 공개키 trust store가 없거나 invalid함 | Packaging / Update / Local development | resolved | [Release package bootstrap trust store is missing or invalid](191_release_package_bootstrap_trust_store_missing_or_invalid.md) |
...
| TS-230 | prove-update-bootstrap이 MacPlatformAgent verify selection을 이후 apply에 상관하지 못함 | Update / Field proof | active | [Platform Agent apply does not share the verified bundle selection](230_platform-agent-verify-apply-selection-correlation.md) |
...
| TS-233 | 표준 제거가 Redis backup API 5초 timeout으로 중단됨 | Uninstall / Data store / Runtime Control | resolved | [Standard uninstall times out while Redis backup completes](233_standard-uninstall-redis-backup-request-timeout.md) |
| TS-234 | 설치된 Host proxy가 bundled nginx dylib 누락으로 반복 종료됨 | Packaging / Host proxy | resolved | [Packaged nginx runtime dylib closure is missing](234_packaged-nginx-runtime-dylib-closure-missing.md) |

...
failure instead of an app-boundary OOM.

## Cause

The upstream VitalServer application owns internal `send_data`, Redis, filter, and export processing. This repository
...
Do not hide upstream app OOM as a generic VM memory problem. The app must fail inside an explicit app boundary, and the
runtime state must preserve the difference between OOM, restart, missing inspection data, HTTP `502`, and stale state.
...
2. `runtime-state.json` includes app container `oomKilled`, `restartCount`, `finishedAt`, `error`, and

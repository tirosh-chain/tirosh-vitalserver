...
VitalServer Helper backup restore fails, but the Helper app does not show a useful failure message. The recovery progress area can also remain empty or keep only the generic running text, so the operator cannot tell whether restore is still running, failed before Redis restore, or failed while validating the backup artifact.
...
- `restore-runtime-data-backup` failed with phase `failed`, reason code `runtime-data-restore-failed`, and the concrete restore error message

...
- 2026-06-05: VM pid file이 이미 없고 VM launchd service도 명시적으로 `notLoaded`인 설치본에서 uninstall이 `vm-process-pid-file-missing`으로 product root 삭제 전 차단될 수 있음을 확인했습니다. Missing pid file 자체는 `stopped`와 다르지만, uninstall readiness는 VM service state를 함께 사용해야 합니다. VM service가 `loaded`, read-failed, permission-denied, unknown이면 계속 차단하고, VM service가 `notLoaded`로 확인된 missing pid는 cleanup 진행을 허용합니다.

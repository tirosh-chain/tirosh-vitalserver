...
7. integration test는 virtual recorder session의 stop 이후 `.vital` 생성, multipart upload, VitalServer식 `success` body, live VitalServer filelist의 non-zero `dtstart/dtend`, webview-compatible device/track/record/montype 확인까지 포함해야 합니다.

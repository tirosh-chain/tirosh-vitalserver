...
현재 macOS Helper app은 SwiftUI presentation, native shell, composition을 담고 있습니다. `RuntimeControlAdapter`는 별도 SwiftPM target이며, `VitalServerHelperApp` 안에서는 composition 파일만 adapter를 import합니다. 다만 아직 같은 Helper app process 안에서 `LocalRuntimeClient`가 host file, privileged command, `vitalserver-vm` CLI를 직접 호출합니다.
...
| Target | 현재 책임 | 전환기 성격 |
|---|---|---|
| `VitalServerHelperApp` | SwiftUI 화면, view model, app composition, native shell | presentation/native shell은 adapter 세부 구현을 모름. composition만 local adapter를 조립 |
| `RuntimeControl` | UI/usecase 입출력 계약, `RuntimeClient`, DTO, enum | 최종 API/client/server가 공유할 계약의 시작점 |
| `RuntimeContracts` | runtime status/progress/health/guest request/result/update bundle/file name 계약 | PWA/API/server/host runtime이 공유할 schema와 enum의 독립 target |
| `RuntimeControlAdapter` | `RuntimeControl`의 macOS local 구현, file/process/CLI adapter | 외부 public surface는 `LocalRuntimeClient` facade 중심. 나중에 Runtime Control API server 쪽 adapter로 이동하거나 축소 가능 |
| `HostRuntimeControl` | `vitalserver-vm` CLI와 runtime workflow 실행 | 현재 host runtime source of truth |
| `RuntimeCore` | evaluator, operation plan, health/recovery policy, port protocol | `RuntimeContracts`에 의존하는 순수 도메인/워크플로 정책 |
...
| Baseline | 현재 상태 |
|---|---|
| UI/usecase 계약 | `RuntimeControl`의 `RuntimeClient`, DTO, enum을 기준으로 고정 |
| Runtime schema 계약 | `RuntimeContracts` target으로 분리해 status/progress/health/update bundle/guest request-result 계약을 독립 |
...
| Local adapter 공개면 | `RuntimeControlAdapter` 외부 public surface는 `LocalRuntimeClient` facade 중심으로 제한 |
| Adapter 내부 구현 | command factory, status/settings/file/log reader, privileged runner는 internal 구현 세부로 유지 |
| API 전환 지점 | `RuntimeController` 또는 future PWA client는 `RuntimeClient` 계약만 보고, `LocalRuntimeClient`는 composition에서 교체 가능 |
...
`RuntimeContracts`와 `RuntimeCore`는 별도 실행 계층이 아니라 `RuntimeControlAdapter`와 `HostRuntimeControl`이 import하는 공유 라이브러리입니다. `RuntimeContracts`는 JSON schema, enum, file name, guest request/result, update bundle manifest처럼 PWA/API/server/host runtime이 함께 알아야 하는 계약을 담습니다. `RuntimeCore`는 그 계약 위의 evaluator, operation plan, recovery policy, port protocol을 담고, macOS process 실행이나 SwiftUI 화면 같은 adapter 책임은 갖지 않습니다.

현재 Helper app 경계는 아래처럼 둡니다. 이 구조는 전환기 SwiftUI app 안에서 ADR 0002의 `RuntimeClient` boundary를 구현한 모양입니다. 코드 배치도 같은 경계를 드러내도록 SwiftPM target을 나눕니다. `VitalServerHelperApp`은 composition/presentation/native shell을 담고, `RuntimeControlAdapter`는 local file/process/CLI adapter를 담습니다. presentation/native shell은 adapter target을 import하지 않고, composition에서만 `LocalRuntimeClient`를 조립합니다.
...
| `RuntimeContracts` | runtime schema 계약 | `Sources/RuntimeContracts/*` | status/progress/health/guest request-result/update bundle/file names, PWA/API/server/host runtime 공유 대상 |
...
| `RuntimeCore` | 공유 정책/workflow | `Sources/RuntimeCore/*` | health/guest evaluator, operation plan, recovery policy, repository/clock/command/file port |
...
| Local control lifecycle facade | `Sources/HostRuntimeControl/Runtime/RuntimeLifecycle.swift` | `runtime install/status/health/verify-bundle/stage-bundle/apply-bundle/rollback/repair-datastore/start-services/stop-services` command를 typed workflow와 runner로 연결 | workflow 내부 단계 구현, DMG/PKG 파일 생성 |
...
| Local control contracts | `Sources/RuntimeContracts/*.swift`, `Sources/RuntimeCore/Ports/*.swift` | runtime status/progress/health/guest request/result/update bundle/port 계약, 닫힌 상태값 enum | host filesystem이나 launchd 직접 호출 |

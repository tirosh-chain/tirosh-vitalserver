...
이 문서는 macOS runtime 문서군을 처음 읽을 때 필요한 큰 지도를 제공합니다.

- Helper app이 어떤 runtime 구조를 제어하는가
- 사용자 화면의 각 탭이 어떤 운영 기능에 대응되는가
- 설치, update, 장애 대응, 개발 실행이 어떤 문서로 이어지는가
- release artifact, update bundle, VM image가 어떻게 구분되는가
- platform-specific host runtime과 Linux guest service stack의 책임이 어디서 갈라지는가
...
|---|---|---|---|
| 신규 현장 설치 | `VitalServerHelper-<version>.dmg` 안의 installer package | `make dist/dmg/release` | [Packaging and Update](packaging.md) |
...
| [Observability](observability.md) | Runtime status, service liveness, event/read model, watchdog/recovery 관측 책임을 볼 때 |
| [Runtime Data Backup](runtime-data-backup.md) | backup artifact schema, compatibility gate, Redis data 보존 정책을 볼 때 |

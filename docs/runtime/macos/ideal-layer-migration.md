...
- `RuntimeManagedOperationGuardComposition`은 `Bootstrap/Composition`에 있으며 `RuntimeManagedOperationGuard`, status reporter, guest bootstrap/runtime-state read ports, bootID stale-result gate, watchdog managed-operation grace window를 조립합니다. HostCLI는 compatibility alias만 유지합니다.
...
- `RuntimeGuestCapabilityCheckerComposition`은 `Bootstrap/Composition`에 있으며 `RuntimeGuestCapabilityChecker`와 guest runtime-state document load port를 조립합니다. HostCLI는 compatibility alias와 concrete guest gateway injection만 유지합니다.
...
- `RuntimeCommandRunner`, `RuntimeGuestGateway`, `RuntimeFileStore`, `RuntimeStorageUsageProviding`, `RuntimeClock`, `RuntimeSleeper`, `RuntimeHTTPProber`, `RuntimeServiceManager`, `RuntimeStatusRepository`, `RuntimeEventRepository` port protocol은 `Application/Ports`에 있고, 기존 `Core` API는 typealias shim으로 유지합니다.
...
- `RuntimeRedisBackupWorkflow`는 `Workflow/RuntimeRepairLifecycle`에 있으며 request preparation, VM start gate, explicit result polling, stale result handling, status writes, timeout을 explicit Host-provided ports로 실행합니다. `RuntimeRedisBackupComposition`은 `Bootstrap/Composition`에서 guest request/result file names, Redis backup result reader, status writer, VM service gate, polling sleep port를 조립합니다.
- `RedisBackupResultReader`는 `Infrastructure/Repositories`에 있으며 result document의 missing, loaded, decode/read failed 상태를 구분해 workflow port에 전달합니다.
- `DatastoreRepairEvaluator`는 `Domain/Policies`에 있고, 기존 `Core` API는 typealias shim으로 유지합니다. `RuntimeDatastoreRepairResultWaiter`, `RuntimeDatastoreRepairRunner`, `RuntimeDatastoreRepairWorkflow`는 `Workflow/RuntimeRepairLifecycle`에 있으며 guest request preparation, VM start/restart selection, explicit guest result wait, proxy/watchdog restart, runtime health gate, status/progress writes를 Host-provided ports로 실행합니다. `RuntimeDatastoreRepairComposition`은 `Bootstrap/Composition`에서 guest request/result gateway, guest run directory, wait timeout, status writer, VM/proxy/watchdog service gates, polling sleep port를 조립합니다.
...
- `GuestActivationEvaluator`와 `GuestShutdownEvaluator`는 `Domain/Policies`에 있고, 기존 `Core` API는 typealias shim으로 유지합니다.

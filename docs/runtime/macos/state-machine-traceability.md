...
| Uninstall | Host runtime | SQLite `workflow_operation_states` (`operation_type=uninstall`) | Domain `RuntimeUninstallTransitionPolicy`, Domain `RuntimeUninstallReadinessPolicy`, Domain `RuntimeUninstallWorkflowOperationStateProjectionPolicy` | `Workflow/RuntimeUninstallLifecycle`, `HostCLI uninstall`, explicit product-root tombstone repository relocation | `DomainRuntimeUninstallTransitionPolicyTests`, `RuntimeUninstallTransitionPolicyTests`, `RuntimeUninstallWorkflowTests`, `RuntimeUninstallWorkflowOperationStateSessionTests` |

...
| Runtime Provider lifecycle | Platform-owned runtime/process state | `GET`/`PUT /platform/runtime-provider` is the API contract. `runtime-state.sqlite.vm_lifecycle` owns durable lifecycle state so the provider can publish before the UI/API process starts; controller, watchdog, VM launcher/delegate, status/health readers, and Golden-rootfs shutdown/compression guards consume the same owner. Golden-rootfs `rootfs-ready` and manifest remain separate Guest compile-proof contracts. Missing, invalid, failed, and loaded stay distinct | Add richer provider reconciliation on top of this owner contract; do not use lifecycle JSON, `runtime-status.json`, or probe output as lifecycle state |

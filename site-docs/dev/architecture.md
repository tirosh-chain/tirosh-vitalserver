...
화면은 명령을 요청하고 결과를 보여줍니다. 실제로 process를 실행하거나 파일을 바꾸거나 VM을 조작하는 일은 Host 쪽에서 처리합니다.

### 2-5. 상태가 올라올 때

상태는 한 줄로 이어지지 않습니다. Runtime 상태 변화와 실패는 status와 event로 남고, recorder/bed 관측은 별도 activity로 정리됩니다. Runtime Control API는 이 자료를 모아서 화면에 전달합니다.

# Reset for Reinstall Command

Reset for Reinstall command는 Vital Server Helper를 새로 설치할 수 없을 때 사용하는 강제 정리
도구입니다. DMG 안에서는 `Troubleshooting Tools/Reset VitalServer Helper for Reinstall.command`
라는 이름으로 제공합니다.

일반 uninstall이 아닙니다. Helper app, runtime service, VM 데이터, 로그, 백업, 설정된 Vital
files directory까지 정리해서 Mac을 다시 설치 가능한 상태로 되돌립니다.

내부 clean uninstall과의 차이는 [Clean Uninstall and Reset for Reinstall](clean-uninstall.md)을
봅니다. 이 command는 정상 제거 UX가 아니라 fresh install blocker를 없애는 recovery tool입니다.
...
단순 app 재시작 문제라면 reset command를 먼저 쓰지 않습니다. Status, Logs, repair
흐름으로 해결 가능한지 먼저 확인합니다.

## 3. 실행 방법

Mac 사용자에게는 DMG 안의 아래 command를 사용하게 안내합니다.
...
2. `Reset VitalServer Helper for Reinstall.command`를 더블클릭합니다.
3. Terminal이 열리면 `sudo` 관리자 암호 입력을 진행합니다.
4. 완료 후 Mac을 재시동합니다.
...
최신 reset command는 이 recovery 경로에서 VM graceful stop을 900초 기다리지
...
sleep-prevention job을 loaded/running으로 보고 있을 수 있습니다. 최신 reset command는 VM pid
...
Terminal 창이 열려 있지만 log가 더 이상 늘어나지 않으면 cleanup worker가 완료 또는 실패 marker를
남기지 못하고 종료된 상태일 수 있습니다. 이 경우 최신 command에서는 아래 marker를 log에
남기고 실패로 종료합니다.
...
Reset command가 성공한 뒤 일반 installer가 아래처럼 막히면, clean uninstall 후 status 관측
...
없을 수 있습니다. 최신 reset command는 이 경우를 성공으로 숨기지 않고 아래처럼 명시적으로
...
Reset command는 Vital Server Helper가 소유한 항목을 제거합니다.

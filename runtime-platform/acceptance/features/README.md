...
- `guest-installed-runtime-evidence.feature` defines C78 first-boot and post-reboot Guest owner evidence through the installed Host facade. Its deterministic counterpart is `../tooling/tests/test_guest_installed_runtime_evidence_runner.py`; only a real Host reboot and installed Guest can produce release evidence.

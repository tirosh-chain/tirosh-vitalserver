...
class LinuxHostPackageComposerTests(unittest.TestCase):
    ...
    def test_composes_declared_release_and_c54_dpkg_handoff_script(self) -> None:
        ...
        self.assertIn('--host-administration-descriptor "/opt/vitalserver-runtime-platform/control/host-agent.local.json"', control["postinst"].decode())
        self.assertIn("--host-administration-timeout-milliseconds 5000", control["postinst"].decode())
        prerm = control["prerm"].decode()
        self.assertIn("--data-disposition preserve-mutable-data", prerm)
        self.assertIn("--host-administration-timeout-milliseconds 5000", prerm)
        ...
        self.assertIn("--host-administration-timeout-milliseconds 5000", postrm)

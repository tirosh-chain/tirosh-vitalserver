...
24. C48 is the release-owned immutable Host product slot contract. It declares the release catalog, release root, `current` activation link, and service-definition hashes separately from persistent data, and carries file identities rather than inferring a release from a receipt, filename, or leftover directory.
25. C49 is the Host Installation Manager's observation only. Receipt absence, a stale release catalog or slot, an unreadable or diverged service definition, and an existing mutable store have different states; none can become a clean-install success by fallback.
26. C50 gives the Host Installation Manager a durable **install** command/journal/receipt language: `preflight`, `quiesce-services`, and `activate-release`. A direct version-changing package install is blocked at preflight and routed to the C25/C26 staged-updater boundary; a same-release repair is a separately admitted operation. Package scripts invoke the manager but do not own this policy. Explicit uninstall/purge semantics are a separate future lifecycle contract, not a package fallback.

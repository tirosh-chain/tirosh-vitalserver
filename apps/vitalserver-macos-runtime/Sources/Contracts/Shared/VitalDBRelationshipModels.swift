public enum VitalDBRelationshipEventType: String, Codable, Equatable, Sendable {
    case handoff
    case duplicateAssignment
    case unlinkedBed
    case unlinkedRecorder
    case staleLink
}

public enum VitalDBRelationshipSeverity: String, Codable, Equatable, Sendable {
    case info
    case warning
    case critical
}

public enum VitalDBBedAssignmentStatus: String, Codable, Equatable, Sendable {
    case online
    case stale
    case offline
}

public struct VitalDBBedAssignmentRecord: Codable, Equatable, Identifiable, Sendable {
    public var id: String
    public let bedID: String
    public let bedName: String?
    public let vrcode: String
    public let startedAt: String
    public let endedAt: String?
    public let lastSeenAt: String?
    public let lastObservedAt: String
    public let status: VitalDBBedAssignmentStatus
    public let patientConnected: Bool?
    public let observationCount: Int

    public init(
        id: String,
        bedID: String,
        bedName: String?,
        vrcode: String,
        startedAt: String,
        endedAt: String?,
        lastSeenAt: String?,
        lastObservedAt: String,
        status: VitalDBBedAssignmentStatus,
        patientConnected: Bool?,
        observationCount: Int
    ) {
        self.id = id
        self.bedID = bedID
        self.bedName = bedName
        self.vrcode = vrcode
        self.startedAt = startedAt
        self.endedAt = endedAt
        self.lastSeenAt = lastSeenAt
        self.lastObservedAt = lastObservedAt
        self.status = status
        self.patientConnected = patientConnected
        self.observationCount = observationCount
    }
}

public struct VitalDBRelationshipEventRecord: Codable, Equatable, Identifiable, Sendable {
    public var id: String
    public let observedAt: String
    public let eventType: VitalDBRelationshipEventType
    public let severity: VitalDBRelationshipSeverity
    public let bedID: String?
    public let bedName: String?
    public let vrcode: String?
    public let previousVrcode: String?
    public let previousBedID: String?
    public let message: String

    public init(
        id: String,
        observedAt: String,
        eventType: VitalDBRelationshipEventType,
        severity: VitalDBRelationshipSeverity,
        bedID: String?,
        bedName: String?,
        vrcode: String?,
        previousVrcode: String?,
        previousBedID: String?,
        message: String
    ) {
        self.id = id
        self.observedAt = observedAt
        self.eventType = eventType
        self.severity = severity
        self.bedID = bedID
        self.bedName = bedName
        self.vrcode = vrcode
        self.previousVrcode = previousVrcode
        self.previousBedID = previousBedID
        self.message = message
    }
}

public struct VitalDBBedAssignmentInsertCommand: Equatable, Sendable {
    public let assignmentID: String
    public let bedID: String
    public let bedName: String?
    public let vrcode: String
    public let startedAt: String
    public let lastSeenAt: String?
    public let lastObservedAt: String
    public let status: VitalDBBedAssignmentStatus
    public let patientConnected: Bool?

    public init(
        assignmentID: String,
        bedID: String,
        bedName: String?,
        vrcode: String,
        startedAt: String,
        lastSeenAt: String?,
        lastObservedAt: String,
        status: VitalDBBedAssignmentStatus,
        patientConnected: Bool?
    ) {
        self.assignmentID = assignmentID
        self.bedID = bedID
        self.bedName = bedName
        self.vrcode = vrcode
        self.startedAt = startedAt
        self.lastSeenAt = lastSeenAt
        self.lastObservedAt = lastObservedAt
        self.status = status
        self.patientConnected = patientConnected
    }
}

public struct VitalDBBedAssignmentUpdateCommand: Equatable, Sendable {
    public let assignmentID: String
    public let bedName: String?
    public let lastSeenAt: String?
    public let lastObservedAt: String
    public let status: VitalDBBedAssignmentStatus
    public let patientConnected: Bool?

    public init(
        assignmentID: String,
        bedName: String?,
        lastSeenAt: String?,
        lastObservedAt: String,
        status: VitalDBBedAssignmentStatus,
        patientConnected: Bool?
    ) {
        self.assignmentID = assignmentID
        self.bedName = bedName
        self.lastSeenAt = lastSeenAt
        self.lastObservedAt = lastObservedAt
        self.status = status
        self.patientConnected = patientConnected
    }
}

public struct VitalDBBedAssignmentCloseCommand: Equatable, Sendable {
    public let assignmentID: String
    public let endedAt: String

    public init(assignmentID: String, endedAt: String) {
        self.assignmentID = assignmentID
        self.endedAt = endedAt
    }
}

public enum VitalDBBedAssignmentProjectionCommand: Equatable, Sendable {
    case insert(VitalDBBedAssignmentInsertCommand)
    case update(VitalDBBedAssignmentUpdateCommand)
    case close(VitalDBBedAssignmentCloseCommand)
}

public struct VitalDBRelationshipProjectionPlan: Equatable, Sendable {
    public let assignmentCommands: [VitalDBBedAssignmentProjectionCommand]
    public let relationshipEvents: [VitalDBRelationshipEventRecord]

    public init(
        assignmentCommands: [VitalDBBedAssignmentProjectionCommand],
        relationshipEvents: [VitalDBRelationshipEventRecord]
    ) {
        self.assignmentCommands = assignmentCommands
        self.relationshipEvents = relationshipEvents
    }
}

public typealias VitalDBRelationshipProjectionPlanning =
    (VitalDBObservationDocument, [String: VitalDBBedAssignmentRecord]) -> VitalDBRelationshipProjectionPlan

...
enum AppConstants {
    ...
    enum Labels {
        ...
        static let preventSystemSleep = "Prevent Mac sleep"
        static let preventSystemSleepHelp = "Keeps the Mac awake while VitalServer services run so the host proxy and VM are not interrupted by idle sleep. The display can still lock or sleep."

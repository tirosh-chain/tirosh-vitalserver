...
public struct RuntimeStatus {
    ...
    public var displayMessage: String? {
        ...
        if !failureReasons.isEmpty {
            lines.append("\(ManagementDefaults.failureReasonsLabel): \(failureReasonText)")
        }
    ...
    private func stepStatusDisplayName(_ status: RuntimeProgressStepStatus) -> String {
        switch status {
        case .pending:
            return ManagementDefaults.waiting
        case .started:
            return ManagementDefaults.running
        case .completed:
            return ManagementDefaults.done
        case .failed:
            return ManagementDefaults.failed.capitalized
        case .skipped:
            return "Skipped"
        case .unknown(let value):
...
public struct RuntimeBackup: Identifiable, Hashable, Sendable {
    ...
    public var sizeText: String {
        guard let sizeBytes else {
            return ManagementDefaults.unknown
        }
...
public struct ProcessResult: Sendable {
    ...
    public var summary: String {
        ...
        if output.isEmpty {
            return exitCode == 0
                ? ManagementDefaults.done
                : ManagementDefaults.commandFailed(exitCode: exitCode)
        }
        return output
    }
}
...
private enum ManagementDefaults {

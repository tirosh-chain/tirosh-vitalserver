...
public enum VMRuntimeConfigComposition {
    ...
    private static func normalizeKernelCommandLine(_ config: inout VMRuntimeConfig) {
        var tokens = config.kernelCommandLine
            .split(separator: " ")
            .map(String.init)
            .filter {
                !$0.hasPrefix("bpf_jit_enable=")
                    && !$0.hasPrefix("root=")
            }
        for token in Constants.BootAssets.requiredCommandLineTokens where !tokens.contains(token) {
            tokens.append(token)
        }

...
final class RuntimeGuestUpdateUseCasesTests: XCTestCase {
    ...
    func testActivationPreservesReadFailureAndTimeoutAsDistinctFailures() {
        ...
        XCTAssertThrowsError(try readFailure.useCase.activateIfNeeded(
            manifest: manifest(artifacts: [.guestDeploy]),
            context: readFailure.context,
            operations: readFailure.operations
        )) { error in
        ...
        XCTAssertThrowsError(try timeout.useCase.activateIfNeeded(
            manifest: manifest(artifacts: [.guestDeploy]),
            context: timeout.context,
            operations: timeout.operations
        )) { error in
            XCTAssertEqual(error as? RuntimeGuestUpdateUseCaseError, .operationFailed("runtime health check failed"))
        }
    ...
    func testShutdownExecutesRequestAndWaitsForReadyResult() throws {
        ...
        try harness.useCase.prepareForUpdate(
            version: "1.2.3",
            context: harness.context,
            operations: harness.operations
        )
    ...
    func testShutdownPreservesGuestFailureReadFailureAndTimeoutMeanings() {
        ...
        XCTAssertThrowsError(try guestFailure.useCase.prepareForUpdate(
            version: "1.2.3",
            context: guestFailure.context,
            operations: guestFailure.operations
        )) { error in
        ...
        XCTAssertThrowsError(try readFailure.useCase.prepareForUpdate(
            version: "1.2.3",
            context: readFailure.context,
            operations: readFailure.operations
        )) { error in
        ...
        XCTAssertThrowsError(try timeout.useCase.prepareForUpdate(
            version: "1.2.3",
            context: timeout.context,
            operations: timeout.operations
        )) { error in
    ...
    final class ActivationHarness {
        let useCase = ActivateRuntimeGuestUpdateUseCase()
        let context = RuntimeGuestActivationUseCaseContext(
        ...
        lazy var operations = RuntimeGuestActivationUseCaseOperations(
    ...
    final class ShutdownHarness {
        let useCase = PrepareRuntimeGuestShutdownUseCase()
        let context = RuntimeGuestShutdownUseCaseContext(
            guestRunDirectory: URL(fileURLWithPath: "/guest/run"),
            waitTimeoutSeconds: 6
        )
        var events: [String] = []
        ...
        lazy var operations = RuntimeGuestShutdownUseCaseOperations(
            requireCapability: { [self] in events.append("capability") },
            createGuestRunDirectory: { [self] directory in events.append("create:\(directory.path)") },

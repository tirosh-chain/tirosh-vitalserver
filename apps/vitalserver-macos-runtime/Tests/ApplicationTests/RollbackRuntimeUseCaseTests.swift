...
final class RollbackRuntimeUseCaseTests: XCTestCase {
    func testRunExecutesRollbackPlanAndWritesHealthyStatus() throws {
        ...
        XCTAssertEqual(harness.executedPlans.map(rollbackPlanStep), RuntimeOperationPlans.rollback.steps)
        XCTAssertEqual(
    ...
    func testRunPublishesFailedProgressWhenStepFails() {
        ...
        XCTAssertEqual(harness.executedPlans.map(rollbackPlanStep), [
    ...
    func testPrepareUsesRequestedBackupAndBuildsContextFromExplicitObservations() throws {
        ...
        let context = try RollbackRuntimeUseCase().preparePreflight(.specificBackup(requestedBackup), operations: operations)
    ...
    func testPrepareFailsBeforeManifestWhenBackupDirectoryIsMissing() {
        ...
        XCTAssertThrowsError(try RollbackRuntimeUseCase().preparePreflight(
    ...
    func testExecuteStepPassesPlannerOutputToExecutionPortWithExplicitContext() throws {
        ...
        let operations = operationsForStep(
            ...
            executeRollbackStepPlan: { executedPlans.append($0) }
        )

        try RollbackRuntimeUseCase().executeStep(
        ...
        XCTAssertEqual(executedPlans, [
            .restoreRuntimeVersion(.restoreBackupVersion(
                source: preflight.backupVersion,
                destination: stepContext.runtimeVersion
            )),
        ])
    }
    ...
    private func operationsForStep(
        ...
        RollbackRuntimeOperations(
            ...
            executeRollbackStepPlan: executeRollbackStepPlan,
            writeStatus: { _, _, _ in },
            writeProgress: { _ in },
            log: { _ in }
        )
...
private final class RollbackHarness {
    ...
    lazy var operations = RollbackRuntimeOperations(
        ...
        executeRollbackStepPlan: { [self] plan in
            executedPlans.append(plan)
            if let stepErrorAt, rollbackPlanStep(plan) == stepErrorAt {
...
private func rollbackPlanStep(_ plan: RollbackRuntimeStepExecutionPlan) -> RuntimeWorkflowStep {
    switch plan {

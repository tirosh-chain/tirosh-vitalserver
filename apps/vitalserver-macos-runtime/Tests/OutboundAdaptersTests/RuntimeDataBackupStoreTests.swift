...
final class RuntimeDataBackupStoreTests: XCTestCase {
    ...
    func testCreateBackupArchivesAllRequiredArtifactsAndWritesManifest() throws {
        ...
        XCTAssertTrue(manifest.artifacts.allSatisfy { $0.state == .archived })
        XCTAssertTrue(manifest.artifacts.allSatisfy { $0.sizeBytes != nil })
        XCTAssertTrue(manifest.artifacts.allSatisfy { $0.sha256?.isEmpty == false })
        ...
        XCTAssertTrue(FileManager.default.fileExists(
            atPath: backup.appendingPathComponent("artifacts/runtime-observability.sqlite").path
        ))
    }

...
testkit/recorder-ingress/replay: require-testkit-runtime
	...
	RECORDER_INGRESS_SEND_DATA_REPLAY_MAX_BYTES_PER_SECOND=20971520 \
	$(PYTHON) scripts/recorder_ingress_compose_e2e.py \
		...
		--replay-min-concurrency 1 \
		--replay-max-concurrency 8
...
testkit/recorder-ingress/runtime-load: require-testkit-runtime
	$(PYTHON) scripts/recorder_ingress_compose_e2e.py \
		--bind-host "$(VITALSERVER_BIND_HOST)" \
		--http-port "$(VITALSERVER_HTTP_PORT)" \
		--recorders 20 \
...
testkit/recorder-ingress/backpressure: require-testkit-runtime
	...
	RECORDER_INGRESS_SEND_DATA_REPLAY_MAX_BYTES_PER_SECOND=1048576 \
	$(PYTHON) scripts/recorder_ingress_compose_e2e.py \
		--compose "$(DOCKER_COMPOSE)" \
		--bind-host "$(VITALSERVER_BIND_HOST)" \
		--http-port "$(VITALSERVER_HTTP_PORT)" \
